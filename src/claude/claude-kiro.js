...
const MODEL_MAPPING = Object.fromEntries(
  Object.entries(FULL_MODEL_MAPPING).filter(([key]) =>
    KIRO_MODELS.includes(key)
  )
);
...
export class KiroApiService {
  constructor(config = {}) {
    ...
    console.log(
      `[Kiro] System proxy ${this.useSystemProxy ? "enabled" : "disabled"}`
    );
    ...
    if (config.KIRO_OAUTH_CREDS_BASE64) {
      try {
        const decodedCreds = Buffer.from(
          config.KIRO_OAUTH_CREDS_BASE64,
          "base64"
        ).toString("utf8");
        ...
        console.info(
          "[Kiro] Successfully decoded Base64 credentials in constructor."
        );
      } catch (error) {
        console.error(
          `[Kiro] Failed to parse Base64 credentials in constructor: ${error.message}`
        );
      }
  ...
  async initializeAuth(forceRefresh = false) {
    if (this.accessToken && !forceRefresh) {
      console.debug(
        "[Kiro Auth] Access token already available and not forced refresh."
      );
      return;
    }
    ...
    const loadCredentialsFromFile = async (filePath) => {
      try {
        ...
        if (error.code === "ENOENT") {
          ...
          console.warn(
            `[Kiro Auth] Failed to parse JSON from ${filePath}: ${error.message}`
          );
        } else {
          console.warn(
            `[Kiro Auth] Failed to read credential file ${filePath}: ${error.message}`
          );
        }
        return null;
      }
      ...
      try {
        ...
        try {
          ...
          if (readError.code === "ENOENT") {
            console.debug(
              `[Kiro Auth] Token file not found, creating new one: ${filePath}`
            );
          } else {
            console.warn(
              `[Kiro Auth] Could not read existing token file ${filePath}: ${readError.message}`
            );
          }
        }
        ...
        console.error(
          `[Kiro Auth] Failed to write token to file ${filePath}: ${error.message}`
        );
      }
    };
    ...
    try {
      ...
      if (this.base64Creds) {
        ...
        console.info(
          "[Kiro Auth] Successfully loaded credentials from Base64 (constructor)."
        );
      ...
      const dirPath = path.dirname(credentialFilePath);
      const targetFileName = path.basename(credentialFilePath);
      ...
      try {
        // 首先尝试读取目标文件
        const targetCredentials = await loadCredentialsFromFile(
          credentialFilePath
        );
        if (targetCredentials) {
          Object.assign(mergedCredentials, targetCredentials);
          console.info(
            `[Kiro Auth] Successfully loaded OAuth credentials from ${credentialFilePath}`
          );
        }
        ...
        for (const file of files) {
          if (file.endsWith(".json") && file !== targetFileName) {
            ...
            if (credentials) {
              ...
              console.debug(
                `[Kiro Auth] Loaded Client credentials from ${file}`
              );
            }
          }
        }
        ...
        console.warn(
          `[Kiro Auth] Error loading credentials from directory ${dirPath}: ${error.message}`
        );
      }
      ...
      if (credentialFilePath && Object.keys(metadataToPersist).length > 0) {
        await saveCredentialsToFile(credentialFilePath, metadataToPersist);
      }
    } catch (error) {
      console.warn(
    ...
    if (forceRefresh || (!this.accessToken && this.refreshToken)) {
      ...
      try {
        ...
        if (response.data && response.data.accessToken) {
          ...
          const tokenFilePath = credentialFilePath;
          const metadataToPersist = buildMetadataPayload();
    ...
    if (!this.accessToken) {
      throw new Error(
        "No access token available after initialization and refresh attempts."
      );
    }
  }
  ...
  buildCodewhispererRequest(
    ...
    if (currentMessage.role === "assistant") {
      console.log(
        "[Kiro] Last message is assistant, moving it to history and creating user currentMessage"
      );
      ...
      if (!currentContent) {
        currentContent =
          currentToolResults.length > 0 ? "Tool results provided." : "Continue";
      }
    }
  ...
  parseEventStreamChunk(rawData) {
    ...
    for (const match of matches) {
      ...
      while (
        ...
        const jsonCandidate = potentialJsonBlock
          .substring(0, searchPos + 1)
          .trim();
        try {
          ...
          if (eventData.name && eventData.toolUseId) {
            ...
            if (eventData.stop) {
              try {
                ...
                console.warn(
                  `[Kiro] Tool call arguments not valid JSON: ${currentToolCallDict.function.arguments}`
                );
              }
  ...
  async callApi(method, model, body, isRetry = false, retryCount = 0) {
    ...
    try {
      ...
      try {
        ...
        if (error.response) {
          console.error(
            `[Kiro] API Error ${error.response.status} (model: ${model}, url: ${requestUrl}):`,
            {
        ...
        if (error.response?.status === 403 && !isRetry) {
          console.log(
            `[Kiro] Received 403 for model ${model}. Attempting smart token refresh and retrying...`
          );
          try {
  ...
  async generateContent(model, requestBody) {
    ...
    try {
      ...
      return this.buildClaudeResponse(
        responseText,
        false,
        "assistant",
        finalModel,
  ...
  parseAwsEventStreamBuffer(buffer) {
    ...
    while (true) {
      ...
      const followupStart = remaining.indexOf(
        '{"followupPrompt":',
        searchStart
      );
  ...
  async *generateContentStream(model, requestBody) {
    ...
    if (this.isExpiryDateNear()) {
      console.log(
        "[Kiro] Token is near expiry, refreshing before generateContentStream request..."
      );
      await this.initializeAuth(true);
    }
    ...
    console.log(
      `[Kiro] Calling generateContentStream with model: ${finalModel} (real streaming)`
    );
  ...
  countTextTokens(text) {
    ...
    try {
      ...
      console.warn(
        "[Kiro] Tokenizer error, falling back to estimation:",
        error.message
      );
    ...
    if (isStream) {
      ...
      events.push({
        ...
        message: {
          ...
          model: model,
          usage: {
            input_tokens: inputTokens, // Kiro API doesn't provide this
      ...
      if (content) {
        ...
        const contentBlockIndex =
          toolCalls && toolCalls.length > 0 ? toolCalls.length : 0;
      ...
      if (toolCalls && toolCalls.length > 0) {
        toolCalls.forEach((tc, index) => {
          ...
          try {
            ...
            console.warn(
              `[Kiro] Invalid JSON for tool call arguments. Wrapping in raw_arguments. Error: ${e.message}`,
  ...
  /**
   * Perform lightweight content requests to determine which mapped models are actually
   * usable with the active credentials. This intentionally skips unknown models that do
   * not exist in MODEL_MAPPING because the backend requires canonical IDs.
   * @param {string[]=} preferredModels - Optional subset of models to validate.
   * @returns {Promise<object>} - Diagnostic report containing availability per model.
   */
  async getModelAccessStatus(preferredModels = undefined) {
    if (!this.isInitialized) {
      await this.initialize();
    }

    const modelsToTest =
      Array.isArray(preferredModels) && preferredModels.length > 0
        ? preferredModels
        : KIRO_MODELS;
    ...
    for (const modelName of uniqueModels) {
      ...
      const record = { model: modelName };

      if (!backendModel) {
        record.status = "unknown";
        record.reason = "MODEL_NOT_MAPPED";
        results.push(record);
        continue;
      }
          ...
          messages: [
            {
              role: "user",
              content:
                "Model availability probe. Please respond with a short acknowledgement.",
            },
          ],
        };
        await this.callApi("", modelName, probeRequest);
        record.status = "available";
        record.backendModel = backendModel;
      } catch (error) {
  ...
  isExpiryDateNear() {
    try {
      ...
      const cronNearMinutesInMillis =
        (this.config.CRON_NEAR_MINUTES || 10) * 60 * 1000;
      ...
      console.error(
        `[Kiro] Error checking expiry date: ${this.expiresAt}, Error: ${error.message}`
      );
