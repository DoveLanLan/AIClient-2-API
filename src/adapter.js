...
export class ApiServiceAdapter {
  constructor() {
    if (new.target === ApiServiceAdapter) {
      throw new TypeError(
        "Cannot construct ApiServiceAdapter instances directly"
      );
    }
  }
...
export class OpenAIApiServiceAdapter extends ApiServiceAdapter {
  ...
  async *generateContentStream(model, requestBody) {
    // The adapter now expects the requestBody to be in the native OpenAI format.
    const stream = this.openAIApiService.generateContentStream(
      model,
      requestBody
    );
...
export class ClaudeApiServiceAdapter extends ApiServiceAdapter {
  ...
  async *generateContentStream(model, requestBody) {
    // The adapter now expects the requestBody to be in the native Claude format.
    const stream = this.claudeApiService.generateContentStream(
      model,
      requestBody
    );
    yield* stream;
...
export class KiroApiServiceAdapter extends ApiServiceAdapter {
  ...
  async generateContent(model, requestBody) {
    ...
    if (!this.kiroApiService.isInitialized) {
      ...
      // 如果初始化后仍然没有有效的 token，尝试智能刷新
      if (
        !this.kiroApiService.accessToken ||
        this.kiroApiService.isExpiryDateNear()
      ) {
        console.log(
          "[Kiro Adapter] Attempting smart token refresh after initialization..."
        );
        try {
          await this.kiroApiService.smartRefreshToken();
        } catch (error) {
          console.warn("[Kiro Adapter] Smart refresh failed:", error.message);
        }
      }
    }
  ...
  async *generateContentStream(model, requestBody) {
    ...
    const stream = this.kiroApiService.generateContentStream(
      model,
      requestBody
    );
    yield* stream;
  ...
  async refreshToken() {
    if (this.kiroApiService.isExpiryDateNear() === true) {
      console.log(
        `[Kiro] Expiry date is near, starting smart token refresh...`
      );
      try {
        ...
        console.log(
          `[Kiro] All token refresh strategies exhausted. Service may be temporarily unavailable.`
        );
